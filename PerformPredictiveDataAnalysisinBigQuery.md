...
- query success rate on penalty kicks by each player
...
- query to analyze shot distance
...
- define a function for calculating the shot distance

CREATE FUNCTION `soccer.GetShotDistanceToGoal887`(x INT64, y INT64)
RETURNS FLOAT64
AS (
 /* Translate 0-100 (x,y) coordinate-based distances to absolute positions
 using "average" field dimensions of 118x80 before combining in 2D dist calc */
 SQRT(
   POW((110 - x) * 118/100, 2) +
   POW((45 - y) * 80/100, 2)
   )
...
- define a function for calculating the shot angle

CREATE FUNCTION `soccer.GetShotAngleToGoal887`(x INT64, y INT64)
RETURNS FLOAT64
AS (
 SAFE.ACOS(
   /* Have to translate 0-100 (x,y) coordinates to absolute positions using
   "average" field dimensions of 118x80 before using in various distance calcs */
   SAFE_DIVIDE(
     ( /* Squared distance between shot and 1 post, in meters */
       (POW(118 - (x * 118/100), 2) + POW(40 + (7.32/2) - (y * 80/100), 2)) +
       /* Squared distance between shot and other post, in meters */
       (POW(118 - (x * 118/100), 2) + POW(40 - (7.32/2) - (y * 80/100), 2)) -
       /* Squared length of goal opening, in meters */
       POW(7.32, 2)
     ...
     (2 *
       /* Distance between shot and 1 post, in meters */
       SQRT(POW(118 - (x * 118/100), 2) + POW(40 + 7.32/2 - (y * 80/100), 2)) *
       /* Distance between shot and other post, in meters */
       SQRT(POW(118 - (x * 118/100), 2) + POW(40 - 7.32/2 - (y * 80/100), 2))
     )
    )
